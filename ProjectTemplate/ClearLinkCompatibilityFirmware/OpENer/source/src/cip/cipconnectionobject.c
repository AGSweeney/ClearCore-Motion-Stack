...
void ConnectionObjectResetSequenceCounts(
  ...
  /*
   * OpENer duplicate suppression compares the next frame to this value (Class 1
   * payload sequence in cipioconnection.c, Class 3 sequence in cpf.c). If this
   * starts at 0, the originator's first sequence value 0 is misclassified as a
   * duplicate and the payload is skipped. Initialize one behind zero in the
   * 16-bit ring so sequence 0 is accepted after Forward Open.
   */
  connection_object->sequence_count_consuming = 0xFFFFU;
}
