...
- For trademarked technology names in docs/comments, use trademark notation consistent with cited third-party source documents on first prominent mention per document (or major section), then use plain text for readability (for example, `EtherNet/IP®` first, then `EtherNet/IP`).
